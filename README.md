...
`med-plugin/` — исправленная версия скиллов из `skills/medical/` (устранены
риски потери данных при обработке inbox, убрана псевдо-метрика достоверности
из `med-merge-history`, добавлена локальная RAG-сверка референсов). Подробности
и установка — в `med-plugin/README.md`.
